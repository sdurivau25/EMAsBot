...
class Bot(Thread) :
    ...
    def __init__(self, client, base, quote, your_base, your_quote, margin_base, margin_quote):
        ...
        self.base_qty = float(your_base + margin_base)
        self.quote_qty = float(your_quote + margin_quote)
        self.min_base = float(client.get_currency(base)['withdrawalMinSize'])
        self.min_quote = float(client.get_currency(quote)['withdrawalMinSize'])
        self.continuer=True
        self.paused = False
    ...
    @property
    def calc_prices_and_emas(self):
        self.prices=[]
        for i in range(0,121):
            self.prices.append(float([(x[2]) for x in client.get_kline_data(symbol=self.pair, '3hour',start=int(time() - 1742400))][i]))    
        # calcul EMAs
         #1 : calcul des sma qui serviront de première ema
        self.sma14=0.0
        for i in range(13,27):
            self.sma14 += self.prices[i]
        self.sma14 = self.sma14/14
        self.sma28=0.0
        for i in range(28,56):
            self.sma28 += self.prices[i]
        self.sma28 = self.sma28/28
        self.sma60=0.0
        for i in range(60,120):
            self.sma60 += self.prices[i]
        self.sma60 = self.sma60/60
         # 2 : Calcul des multiplicateurs
        self.m14 = 2/(14+1)
        self.m28= 2/(28+1)
        self.m60= 2/(60+1)
         # 3 : Calcul des ema
        self.ema14=self.sma14
        for i in range(1,14):
            self.ema14 = (self.prices[14-i]-self.ema14)*self.m14 + self.ema14
        print(self.ema14)
        self.ema28=self.sma28
        for i in range(1,28):
            self.ema28 = (self.prices[28-i]-self.ema28)*self.m28 + self.ema28
        print(self.ema28)
        self.ema60=self.sma60
        for i in range(1,60):
            self.ema60 = (self.prices[60-i]-self.ema60)*self.m60 + self.ema60
        print(self.ema60)  

    def analyze_market(self):
        if self.ema14>self.ema28 and self.ema28>self.ema60 :
            self.full_long = True
            self.log("We'll go long")
        else :
            self.full_long = False
        if self.ema14<self.ema28 and self.ema28<self.ema60 :
            self.full_short = True
            self.log("We'll go short")
        else :
            self.full_short = False
        if not self.full_long and not self.full_short :
            if self.base_qty < self.margin_qty :
                self.stop_long=True
                self.log("We'll stop longing")
            elif self.quote_qty < self.margin_quote :
                self.stop_short=True
                self.log("We'll stop shorting")
            else :
                self.log('Impossible situation')
            
    def check_to_do(self):
        if self.full_long:
            if self.base_qty > self.min_base :
                self.buy_all=True
                self.order_size=0.99*(self.base_qty)
                self.log("Let's buy them all")
            else :
                self.buy_all=False
        if self.full_short:
            if self.quote_qty > self.min_quote :
                self.sell_all=True
                self.order_size=0.99*float(self.quote_qty)
                self.log("Let's sell all!")
            else :
                self.sell_all=False
        if self.stop_long :
            if self.quote_qty > self.margin_quote and (self.quote_qty-self.margin_quote)>self.min_quote:
                self.order_size=0.99*float(self.quote_qty-self.margin_quote)
                self.sell_long=True
                self.log("Let's sell the long !")
            else :
                self.sell_long=False
        if self.stop_short :
            if self.base_qty>self.margin_base and (self.base_qty-self.margin_base)>self.min_base:
                self.order_size=0.99*(self.base_qty-self.margin_base)
                self.sell_short=True
                self.log("let's sell the short !")
            else :
                self.sell_short=False          
        if not self.full_long and not self.full_short and not stop_long and not self.stop_short :
            self.log('Nothing occurs')
            
    def place_order(self):
        if self.buy_all or self.sell_short :
            self.log('Gonna place a buy order')
            client.create_market_order(self.paire, Client.SIDE_BUY, funds=self.order_size)
            self.lastorder = client.get_orders(symbol=self.paire)['items'][0]
            self.log('Bought {}{} at price {}, using {}{}'.format(self.lastorder['dealSize'],self.quote, self.lastorder['price'], self.lastorder['dealFunds'], self.base)
        elif self.sell_all or self.sell_long :
            self.log('Gonna place a sell order')
            client.create_market_order(self.paire, Client.SIDE_SELL, size=self.order_size)
            self.lastorder = client.get_orders(symbol=self.paire)['items'][0]
            self.log('Sold {}{} at price {}, using {}{}'.format(self.lastorder['dealSize'],self.quote, self.lastorder['price'], self.lastorder['dealFunds'], self.base)
            
    def conclude(self)
        sleep(1.5)
        if self.buy_all or self.sell_short:
            self.base_qty = self.base_qty-float(self.lastorder['dealFunds'])
            self.quote_qty = self.quote_qty+float(self.lastorder['dealSize'])
        if self.sell_all or self.sell_long :
            self.base_qty = self.base_qty+float(self.lastorder['dealFunds'])
            self.quote_qty = self.quote_qty-float(self.lastorder[0]['dealSize'])
