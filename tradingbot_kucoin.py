...
class Indicators(Thread):
    ...
    def __init__(self, pair, ema20, ema45, ema130, client=client0):
    ...
    def get_2h_prices(self):
        self.prices = [float(x[2]) for x in client.get_kline_data(symbol=self.pair, kline_type='2hour', start=int(time.time() - 1879200))]   
...
class Bot(Thread) :
    ...
    def __init__(self, client, bot_token, bot_chatID, base, quote, your_base, your_quote, margin_base, margin_quote, indicators, bypass=False):
        Thread.__init__(self)
        self.client = client
    ...
    def place_order(self):
        if self.buy_all or self.sell_short :
            ...
            sleep(1.5)
            self.lastorder = self.client.get_orders(symbol=self.paire)['items'][0]
            self.telegram_bot_sendtext('Sold {}{} at price {}, using {}{}'.format(self.lastorder['dealSize'],self.quote, self.lastorder['price'], self.lastorder['dealFunds'], self.base))
    ...
    def run(self):
        ...
        self.telegram_bot_sendtext('Bot is ready and looking for entry point ; this can take days, be patient ! It is worth waiting.')
        self.analyze_market()
        if self.bypass :
            ...
            while self.full_long :
                sleep(10)
                self.analyze_market()
            self.log('Bot found his entry point')
            self.telegram_bot_sendtext('Bot found his entry point')
            self.log('Bot is ready...')
            while self.continuer:
                sleep(2.0)
                while self.paused:
...
""")
        ...
        bot_token = input('@Trading_Notif_Bot token ?')
        ...
        if pair not in indicators:
            indicators[pair] = Indicators(pair='{}-{}'.format(quote, base))
        n=Bot(client, bot_token, bot_chatID, base, quote, your_base, your_quote, margin_base, margin_quote, indicators=indicators[pair])
        n.start()
        bots.append(n)
        ...
        bot_token = input('@TradingLogsBot token ?')
        ...
        if paire not in indicators:
            indicators['{}-{}'.format(quote, base)] = Indicators(pair='{}-{}'.format(quote, base))
        n=Bot(client, bot_token, bot_chatID, base, quote, your_base, your_quote, margin_base, margin_quote, bypass = True)
        n.start()
        bots.append(n)
