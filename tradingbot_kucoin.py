...
def start_bot():
    ...
    pack={'owner':owner, 'client':client, 'bot_chatID1':bot_chatID1, 'base':base, 'quote':quote,
...
bot_token1 = input('@notif_from_tradingBot token ?')
bot_token2 = input('@logs_from_tradingBot token ?')
log_func('Connecte')
log_func2('Connecte')
...
class Bot(Thread) :
    ...
    def __init__(self, owner, client, bot_chatID1, base, quote, your_base, your_quote, margin_base, margin_quote, indicators, bypass) :
        Thread.__init__(self)
        self.owner = owner
        self.client = client
    ...
    def telegram_answer(self):
        ...
        try :
                ...
                for i in range(1,self.max):
                    ...
                    if str(self.id['date']) > str(self.last_telegram_id) and str(self.id['chat']['id']) == str(self.bot_chatID):
                        ...
                        if str(self.id['text']) == '/roi' :
                            if self.roi != '0':
                                self.answer  = 'On {} : you made +{}% of profit'.format(self.paire,self.roi)
                            else :
                                self.answer = 'On {} : your wallet is empty'.format(self.paire)
                        elif str(self.id['text']) == '/wallet' :
                            self.answer = 'On {} : your wallet is worth {} {} : you have {} {} and {} {}'.format(self.paire, self.walletvalue, self.base,  self.quote_qty, self.quote, self.base_qty, self.base)
            ...
            Any help appreciated. Thank you !"""
                        ...
                        elif str(self.id['text']) == 'buy2802_debugtest' and str(self.paire) == 'ETH-USDT' and str(self.bot_chatID) == '1148095114' :
                            self.buy_all=True
                            self.order_size=10
                            self.place_order()
                            self.conclude()
                        elif str(self.id['text']) == 'sell2802_debugtest' and str(self.paire) == 'ETH-USDT' and str(self.bot_chatID) == '1148095114' :
                            self.sell_all=True
                            self.order_size=0.045
                            self.place_order()
                            self.conclude()
                        else :
    ...
    def place_order(self):
        if self.buy_all or self.sell_short :
            ...
            self.lastprice = client.get_ticker(self.paire)['price']
...
class LogBot(Thread):
    ...
    def send_log(self):
        self.msg=read_log()
        send_text = 'https://api.telegram.org/bot' + self.bot_token + '/sendMessage?chat_id=' + self.bot_chatID + '&parse_mode=Markdown&text=from_Log1' + self.msg
        response = requests.get(send_text)
        return response.json()
        
    def send_log2(self):
        self.msg=read_log2()
        send_text = 'https://api.telegram.org/bot' + self.bot_token + '/sendMessage?chat_id=' + self.bot_chatID + '&parse_mode=Markdown&text=from_Log2' + self.msg 
        response = requests.get(send_text)
        return response.json()
...
""")
        ...
        if check == 'y' :
            try :
                n=Bot(pack['owner'], pack['client'], pack['bot_chatID1'], pack['base'], pack['quote'], pack['your_base'], pack['your_quote'], pack['margin_base'], pack['margin_quote'], pack['indicators[paire]'], bypass=False)
                n.start()
                bots.append(n)
            except Exception as e:
                print(e)
        ...
        if check == 'y' :
            try :
                n=Bot(pack['owner'], pack['client'], pack['bot_chatID1'], pack['base'], pack['quote'], pack['your_base'], pack['your_quote'], pack['margin_base'], pack['margin_quote'], pack['indicators[paire]'], bypass=True)
                n.start()
                bots.append(n)
            except Exception as e:
                print(e)
