...
class Indicators(Thread):
    ...
    def get_2h_prices(self):
        a =int(time.time())
        self.prices = [float(x[2]) for x in client.get_kline_data(symbol=self.paire, kline_type='1min', start=int(a-20000), end=int(a))]   
        print(self.prices)
    ...
    def calc_2h_emas(self):        
        ...
        def return_ema20(self):
            return self.ema20
        def return_ema45(self):
            return self.ema45
        def return_ema130(self):
            return self.ema130
        ...
        def run(self):
            while True :
                self.get_2h_prices()
                self.calc_2h_emas()
                self.ema20 = self.return_ema20()
                self.ema45 = self.return_ema45()
                self.ema130 = self.return_ema130()
                sleep(5)
...
""")
        ...
        n=Bot(owner, client, bot_chatID1, base, quote, your_base, your_quote, margin_base, margin_quote, indicators=indicators[paire], bypass=False)
        n.start()
        bots.append(n)
        ...
        if paire not in indicators:
            indicators[paire] = Indicators(paire='{}-{}'.format(quote, base))
        n=Bot(owner, client, bot_chatID1, base, quote, your_base, your_quote, margin_base, margin_quote, indicators=indicators[paire], bypass=True)
        n.start()
        bots.append(n)
